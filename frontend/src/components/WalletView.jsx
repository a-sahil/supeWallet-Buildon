...
function WalletView({
  ...
  const doSomething = async () => {
    try {
      const intent = value; // Use the input value entered by the user
      // if (!intent1) {
      ...
      if (intent === "send 0.0002 eth to 0xe8FA5C28CA55B1DFBb6BCDBAcE5A6F22F487d662" || intent === "send") {
        // Use the extracted address and amount
        const address = "0xe8FA5C28CA55B1DFBb6BCDBAcE5A6F22F487d662";
        ...
        intent === "tokenizeDocuments" ||
        intent === "tokenize my docs" ||
        intent === "tokenize_documents" 
  ...
  async function getAccountTokens() {
    setFetching(true);
    const res = await axios.get(`http://localhost:3009/getTokens`, {
      params: {
        userAddress: wallet,
        chain: selectedChain,
      },
